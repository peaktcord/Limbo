...
static const S60Face *faceFor(Font *font) {
    if (font == nullptr) return &kSmallBold;
    int32_t size = font->getSize();
    int32_t style = font->getStyle();
    if (size == 16) return (style & 2) ? &kLargeItalic : &kLargeBold;
    if (size == 0) return (style & 1) ? &kMediumBold : &kMediumPlain;
...
Surface *surfaceFor(Image *image) {
    ...
    Surface *surface = session_new<Surface>();
...
SoftGraphics::SoftGraphics(Surface *target) : target_(target) { resetClip(); }
...
void SoftGraphics::onClip() {
    ...
    int64_t x = (int64_t)Graphics::clipX_;
    int64_t y = (int64_t)Graphics::clipY_;
...
int32_t SoftGraphics::drawGlyph(const S60Face *face, uint16_t ch, int32_t x, int32_t top,
                                uint32_t argb) {
    uint32_t code = ch;
    if (code < kS60FirstCode || code > kS60LastCode) code = '?';
...
void SoftGraphics::drawString(const std::string &text, int32_t x, int32_t y, int32_t anchor) {
    ...
    int32_t width = font == nullptr ? 0 : font->stringWidth(text);
    int32_t ascent = 0;
    for (int32_t n = 0; n <= kS60LastCode - kS60FirstCode; ++n) {
        if (face->glyphs[n].ascent > ascent) ascent = face->glyphs[n].ascent;
    }
    applyAnchor(anchor, width, height, ascent, &x, &y);
    uint32_t argb = opaque(getColor());
    int32_t baseline = y + ascent;
    for (int32_t n = 0; n < voyage::text::length(text); ++n) {
        uint16_t ch = voyage::text::byte_at(text, n);
        uint32_t code = ch;
        if (code < kS60FirstCode || code > kS60LastCode) code = '?';
        const S60Glyph &glyph = face->glyphs[code - kS60FirstCode];
        x += drawGlyph(face, (uint16_t)code, x, baseline - glyph.ascent, argb);
    }
}

void SoftGraphics::drawChar(uint16_t ch, int32_t x, int32_t y, int32_t anchor) {
    drawString(std::string(std::string(1, (char)ch)), x, y, anchor);
}
...
void SoftGraphics::drawImage(Image *image, int32_t x, int32_t y, int32_t anchor, int32_t manipulation) {
    ...
    const bool mirror = (manipulation & IMAGE_FLIP_HORIZONTAL) != 0;
    for (int32_t sy = 0; sy < source->height; ++sy) {
        ...
        for (int32_t sx = 0; sx < source->width; ++sx) {
            int32_t srcX = mirror ? (source->width - 1 - sx) : sx;
            blendPixel(x + sx, dy, srcRow[srcX]);
        }
    }
}

SoftRenderer::SoftRenderer(Surface *screen) : screen_(screen) {}

Graphics *SoftRenderer::imageGraphics(Image *image) {
    Surface *surface = surfaceFor(image);
    std::unique_ptr<SoftGraphics> &slot = image_graphics_[image];
    if (slot == nullptr) {
        slot = std::make_unique<SoftGraphics>(surface);
    } else {
        slot->bind(surface);
    }
    return slot.get();
...
void SoftRenderer::forgetImages() { image_graphics_.clear(); }

void SoftRenderer::beginPaint() {
    mutex_.lock();
    screen_.resetClip();
}

void SoftRenderer::endPaint() { mutex_.unlock(); }
